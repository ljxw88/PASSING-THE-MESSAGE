...
package Generic_Router is
   ...
   task type Router_Task (Task_Id  : Router_Range := Draw_Id) is
      ...
      entry Broadcast (Message : Routing_Links; ID : Router_Range; SDResponse : Boolean);

      entry Pass_Message (Message : out Passing_Message);

   end Router_Task;

   function Client2Mailb (Message : Messages_Client; HopC : Natural; SenderId : Router_Range) return Messages_Mailbox;

   function Client2Pass (Message : Messages_Client; HopC : Natural; SenderId : Router_Range) return Passing_Message;

   function Pass2Mailb (Message : Passing_Message) return Messages_Mailbox;
