...
package Generic_Message_Structures is
   ...
   type Passing_Message is record
      Sender : Router_Range := Router_Range'Invalid_Value;
   ...
   type Routing_Link_Base is record
      To_Id : Router_Range := Router_Range'Invalid_Value;
      To_Is_Shutdown : Boolean := False;
      To_Distance : Natural := INF;
   end record;

   type Routing_Links is array (Router_Range) of Routing_Link_Base;
